...
The function `fastFMM::fui` requires that data be presented in a long format, i.e., columns correspond to locations on the functional domain $1, 2, \dots, L$ and rows correspond to longitudinal observations $t_{i, j}$. For example, within `fastFMM`, we provide the pre-formatted dataset `lick`, sourced from [Jeong et al. (2022)](https://doi.org/10.1126/science.abq6740). 

```{r read lick}
lick <- read.csv("../data/lick.csv")
```

The columns `id, session, trial` correspond to trial-level covariates. The functional outcome of `photometry` is stored in the columns `photometry_1, photometry_2, ..., photometry_50` and the functional covariate `lick` is stored in the columns `lick_1, lick_2, ..., lick_50`.

Alternatively, functional outcomes and covariates can be stored `AsIs` as matrix columns. 

```{r lick asis}
library(dplyr)
lick_asis <- lick %>%
  mutate(
    photometry = AsIs(dplyr::select(lick, photometry_1:photometry_50))
  ) %>%
  select(-photometry_1:-photometry_50, -lick_1:-lick_50)
```
