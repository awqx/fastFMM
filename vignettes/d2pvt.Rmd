...
To incorporate latency as a scalar covariate, we specified the **non-concurrent** model
...
- $\texttt{Lat}_{i, j}$: the latency, in seconds, until mouse $i$ in trial $j$ received the food reward.
...
- $\gamma_{i, 0}$: the random intercept for mouse $i$, interpreted as the average photometry signal of mouse $i$ on trials with water reward and average latency, and
- $\gamma_{i, 1}$ the random slope for mouse $i$, interpreted as the subject-specific association between signal and the SMS reward for mouse $i$ on average latency, 

and the fixed effect coefficients are

- $\beta_0 (s)$: the intercept when $\texttt{SMS}_{i, j} = \texttt{Lat}_{i, j} = 0$ at timepoint $s$, i.e., the mean signal at $s$ on water trials when a mouse responds with an average latency,
...
- $\beta_3 (s)$: the difference $A - B$ between A) the change in mean signal associated with a one second increase in latency on SMS trials and B) $\beta_2$ at timepoint $s$.
...
As mentioned previously, we can convert the scalar covariate of `Lat` into the functional covariate `RZ`, where `RZ` is a function of the time point $s$ that is 1 when the mouse has received a reward by time point $s$ and 0 otherwise. We specified the **concurrent** model as
...
- $\beta_0(s)$: the intercept when $\texttt{RZ}_{i, j} (s) = \texttt{SMS}_{i, j} = 0$, i.e., the mean signal on a water trial at a timepoint $s$ when the reward has not yet been received, 
...
**Note**: The concurrent model cannot fit timepoints with zero variance, so we fit to the data `d2pvt_nonzero`, a subset of `d2pvt`. In this case, the functional covariate `RZ` is the same across all mice at the beginning and end of the trial.
...
concurrent <- fastFMM::fui(
  ...
  d2pvt_nonzero, 
  # Uncomment this to make the model fit faster
  parallel = TRUE,
  concurrent = TRUE, 
  silent = TRUE
)
...
Let us select some arbitrary mouse $i = 1$ and assume that it runs trial $j = 10$ slower than average, i.e., $\texttt{Lat}_{1, 10} = c$, where $c$ is some positive constant. Let us also consider mouse $i = 2$ that runs trial $j = 10$ at the average speed, i.e., $\texttt{Lat}_{1, 10} = 0$. Assume both Mouse 1 and 2 are in water trials, i.e., $\texttt{SMS}_{i, 10} = 0$. Let us consider $s = 3$, which is close to the average trial length, and write the photometry signals for these mice as:

$$
Y_{1, 10}(3) = \beta_0 (3) + c \beta_2 (3),  \quad
Y_{2, 10}(3) = \beta_0 (3).
$$

It is important to note that the left equation (slow mouse) reflects the photometry signal of a mouse that has not yet reached the reward while the right equation is a mouse that has just received the reward. Thus, the negative contribution of $c \beta_2 (3)$ (recall: $\beta_2 (3) < 0$) could be interpreted as correcting for the intercept. 

The shape of the coefficient curve for the interaction `SMS:Lat` also shows a dip. However, it is not the case that this implies that slow mice exhibit decreased signal association with the SMS reward, on average. Consider some trial $j = 11$ where both Mouse 1 and 2 receive SMS. Again, let Mouse 1 run slower than average. We can write the photometry signals for these mice as

$$
Y_{1, 11}(3) = \beta_0 (3) + \beta_1 (3) + c \beta_2 (3) + c \beta_3 (3),  \\
Y_{2, 11}(3) = \beta_0 (3) + \beta_1 (s).
$$
Again, at timepoint $s = 3$, Mouse 1 has not yet received the reward. Thus, if we interpret $\beta_1 (3)$ as the mean signal change associated with SMS at time $s = 3$, Mouse 1 would somehow have prior knowledge of the reward. Thus, the shape of the $\beta_3 (s)$ (corresponding to `SMS:Lat` in the plot) interaction functional coefficient can be explained as a correction for the strongly positive $\beta_2 (s)$ (`SMS`) main effect functional coefficient for slower-than-average mice. 

### Concurrent model coefficients

Like with the non-concurrent model, we can write out the model specifications and consider different cases. The concurrent model (without random effects or noise) is copied below:
...
At first, the concurrent model's coefficients might be surprising, as it seems that `SMS` has no contribution to the signal change within a trial. Let us consider Mouse 1 that tends to always run a slow time. Let Mouse 1 receive water in trial $j = 10$ and SMS in trial $j = 11$. Consider time $s = 3$, when Mouse 1 has not yet received a reward, i.e., $\texttt{RZ}_{1, 10} (3) = 0$. The signal is thus $Y_{1, 10} (3) = \beta_0 (3)$. In general, receiving SMS will only make a difference to the signal through the interaction term `SMS:RZ`. Thus, it is a good sanity check that $\beta_2 (s) \approx 0$ for all values of $s$, since $\beta_2 (s) \neq 0$ would imply that a mouse could know the reward before receiving it. 

Now, let us consider Mouse 1 in Trial 10 and 11 at time point $s = 6$, when it receives a reward. We have

$$
Y_{1, 10}(6) = \beta_0 (6) + \beta_2 (6),  \\
Y_{1, 11}(6) = \beta_0 (6) + \beta_1 (6) + \beta_2 (6) + \beta_3 (6) \approx \beta_0 (6)  + \beta_2 (6) + \beta_3 (6).
$$
